...
namespace AElf.OS.Network.Grpc
{
    public class GrpcPeerPool : IPeerPool
    {
        ...
        public IPeer FindPeerByPublicKey(string publicKey)
        {
            ...
            _authenticatedPeers.TryGetValue(publicKey, out var p);
        ...
        {
            var localPubKey = AsyncHelper.RunSync(_accountService.GetPublicKeyAsync).ToHex();
        ...
        {
            if (_authenticatedPeers.TryRemove(publicKey, out var removed))
            {
        ...
        private async Task<bool> DialAsync(string ipAddress)
        {
            ...
            {
                metadata.Add(GrpcConsts.PubkeyMetadataKey,
        ...
        private async Task<Handshake> BuildHandshakeAsync()
        {
            ...
            var sig = await _accountService.SignAsync(Hash.FromMessage(nd).ToByteArray());
