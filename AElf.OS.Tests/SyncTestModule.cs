    ...
    public class SyncTestModule : AElfModule
    {
        public override void ConfigureServices(ServiceConfigurationContext context)
        {
            ...
            context.Services.AddSingleton(p =>
            {
                ...
                blockchainService.Setup(bs => bs.GetChainAsync())
                    .Returns(Task.FromResult(new Chain {LastIrreversibleBlockHeight = 2}));

                blockchainService.Setup(bs =>
                        bs.AttachBlockToChainAsync(It.IsAny<Chain>(), It.Is<Block>(b => b.Equals(block))))
                    .Returns(Task.FromResult(BlockAttachOperationStatus.NewBlockLinked));

                blockchainService.Setup(bs =>
                        bs.AttachBlockToChainAsync(It.IsAny<Chain>(), It.Is<Block>(b => b.Equals(block2))))
                    .Returns(Task.FromResult(BlockAttachOperationStatus.NewBlockNotLinked));

                return blockchainService.Object;
            });

            context.Services.AddSingleton(p => Mock.Of<IBlockchainExecutingService>());
        }
    }
}