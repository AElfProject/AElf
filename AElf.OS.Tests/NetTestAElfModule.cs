...
namespace AElf.OS
{
    ...
    public class NetTestAElfModule : AElfModule
    {
            ...
            context.Services.AddSingleton(o =>
            {
                ...
                peerMock.Setup(p => p.GetBlocksAsync(It.IsAny<Hash>(), It.IsAny<int>()))
                    .Returns<Hash, int>((hash, cnt) =>
                    {
                        ...
                        var selection = _blockList.Where(b => b.Height > requested.Height).OrderBy(b => b.Height)
                            .Take(cnt).ToList();
        ...
        public override void OnApplicationInitialization(ApplicationInitializationContext context)
        {
            ...
            {
                var newBlock = AsyncHelper.RunSync(() => genService.GenerateBlockBeforeExecutionAsync(
                    new GenerateBlockDto
                    {
                ...
                var newNewBlock = AsyncHelper.RunSync(() => exec.ExecuteBlockAsync(newBlock.Header,
                    new List<Transaction>(), new List<Transaction>(), CancellationToken.None));
