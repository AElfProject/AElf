...
namespace AElf.Kernel.Tests.Concurrency.Metadata
{
    ...
    public class ChainFunctionMetadataTemplateTest
    {
        ...
        public async Task<ChainFunctionMetadataTemplate> TestTryAddNewContract()
        {
            ...
            var groundTruthMap =
                new Dictionary<string, Dictionary<string, FunctionMetadataTemplate>>(cfts.ContractMetadataTemplateMap);
            //Throw exception because 
            var exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractA))).Result;
            Assert.True(exception.Message.Contains("Unknow reference of the foreign target"));

            //Not changed
            Assert.Equal(groundTruthMap, cfts.ContractMetadataTemplateMap);
            ...
            groundTruthMap.Add("AElf.Kernel.Tests.Concurrency.Metadata.TestContractB",
                new Dictionary<string, FunctionMetadataTemplate>(new[]
                {
                    new KeyValuePair<string, FunctionMetadataTemplate>(
                        "${this}.Func0",
                        new FunctionMetadataTemplate(
            ...
            groundTruthMap.Add("AElf.Kernel.Tests.Concurrency.Metadata.TestContractA",
                new Dictionary<string, FunctionMetadataTemplate>(new[]
                {
                    new KeyValuePair<string, FunctionMetadataTemplate>(
                        "${this}.Func0(int)",
            ...
            Assert.Equal(cfts.ContractMetadataTemplateMap, newCFTS.ContractMetadataTemplateMap);
        ...
        public async Task<ChainFunctionMetadataTemplate> TestFailCases(ChainFunctionMetadataTemplate cfts)
        {
            ...
            var exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractD))).Result;

            Assert.True(exception.Message.Contains("Duplicate name of field attributes in contract"));

            exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractE))).Result;
            Assert.True(
                exception.Message.Contains("Duplicate name of smart contract reference attributes in contract "));

            exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractF))).Result;
            Assert.True(exception.Message.Contains("Unknown reference local field ${this}.resource1"));

            exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractG))).Result;
            Assert.True(exception.Message.Contains("Duplicate name of function attribute"));

            exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractH))).Result;
            Assert.True(exception.Message.Contains("contains unknown reference to it's own function"));

            exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractI))).Result;
            Assert.True(exception.Message.Contains("contains unknown local member reference to other contract"));

            exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractJ))).Result;
            Assert.True(exception.Message.Contains("is Non-DAG thus nothing take effect"));

            exception = Assert
                .ThrowsAsync<FunctionMetadataException>(() => cfts.TryAddNewContract(typeof(TestContractK))).Result;
            Assert.True(
