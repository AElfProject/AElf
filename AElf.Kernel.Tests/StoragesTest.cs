...
namespace AElf.Kernel.Tests
{
    ...
    public class StoragesTest
    {
        ...
        public async Task OneBlockDataTest()
        {
            ...
            await dataProvider.SetAsync(key, new Data {Value = ByteString.CopyFrom(data)});
            var getData = await dataProvider.GetAsync(key);
            
            Assert.True(data.SequenceEqual(getData.Value));
            ...
            await subDataProvider.SetAsync(key, new Data {Value = ByteString.CopyFrom(data2)});
            var getData2 = await subDataProvider.GetAsync(key);
            
            Assert.True(data2.SequenceEqual(getData2.Value));

            var data3 = new byte[] {4, 3, 2, 1};

            await subDataProvider.SetAsync(key, new Data {Value = ByteString.CopyFrom(data3)});
            var getData3 = await subDataProvider.GetAsync(key);
            
            Assert.True(data3.SequenceEqual(getData3.Value));
        }
        ...
        public async Task TwoBlockDataTest()
        {
            ...
            var key = new Hash("testkey".CalculateHash());
            await subDataProvider.SetAsync(key, new Data {Value = ByteString.CopyFrom(data)});
            
            var getDataFromHeight1 = await subDataProvider.GetAsync(key);
            
            Assert.True(data.SequenceEqual(getDataFromHeight1.Value));
            ...
            Assert.True(getDataFromHeight1.Equals(getDataFromHeight2));

            var data2 = new byte[] {1, 2, 3, 4, 5};
            await subDataProvider.SetAsync(key, new Data {Value = ByteString.CopyFrom(data2)});
            getDataFromHeight2 = await subDataProvider.GetAsync(key);
            
            Assert.False(getDataFromHeight1.Equals(getDataFromHeight2));

            var getDataFromHeight1ByBlockHash = await subDataProvider.GetAsync(key, genesisBlockHash);
            
            Assert.True(getDataFromHeight1.Equals(getDataFromHeight1ByBlockHash));
        }
        ...
        public async Task MultiBlockDataTest()
        {
            ...
            var key = new Hash("testkey".CalculateHash());
            await subDataProvider.SetAsync(key, new Data {Value = ByteString.CopyFrom(data1)});
            ...
            Assert.True(data1.SequenceEqual(getData1.Value));
            //And the ability to get data of current WorldState.(not equal to previous data)
            var getData2 = await subDataProvider.GetAsync(key);
            Assert.False(data1.SequenceEqual(getData2.Value));
            ...
            //And the ability to get data of current WorldState.(not equal to previous data)
            var getData3 = await subDataProvider.GetAsync(key);
            Assert.False(data1.SequenceEqual(getData3.Value));
            Assert.False(data2.SequenceEqual(getData3.Value));
            Assert.True(data3.SequenceEqual(getData3.Value));
        }
