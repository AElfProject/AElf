...
public sealed class BlockChainAppServiceTest : WebAppTestBase
{
    ...
    public async Task Get_TransactionResults_Success_Test()
    {
        ...
        for (var i = 0; i < 20; i++) transactions.Add(await _osTestHelper.GenerateTransferTransaction());
    ...
    public async Task Get_Block_By_BlockHeight_Success_Test()
    {
        var chain = await _blockchainService.GetChainAsync();
        var transactions = new List<Transaction>();
        for (var i = 0; i < 3; i++) transactions.Add(await _osTestHelper.GenerateTransferTransaction());
        ...
        response.Header.MerkleTreeRootOfTransactionState.ShouldBe(
            block.Header.MerkleTreeRootOfTransactionStatus.ToHex());
    ...
    public async Task Get_Block_State_FewBlocks_Later_Test()
    {
        ...
        for (var i = 0; i < 10; i++) await _osTestHelper.MinedOneBlock();
    ...
    public async Task SendRawTransaction_Success_Test()
    {
        ...
        sendTransactionResponse.Transaction.RefBlockPrefix.ShouldBe(BlockHelper.GetRefBlockPrefix(chain.BestChainHash)
            .ToBase64());
    ...
    {
        var hex = "5a7d71da020cae179a0dfe82bd3c967e1573377578f4cc87bc21f74f2556c0ef";
    ...
    {
        var transactionId = "InvalidTransactionId";
        var errorResponse = await GetResponseAsObjectAsync<WebAppErrorResponse>(
            $"/api/blockChain/merklePathByTransactionId?transactionId={transactionId}",
            expectedStatusCode: HttpStatusCode.Forbidden);
    ...
    public async Task TransactionValidationStatusChangedEventHandler_Test()
    {
        ...
        await _transactionValidationStatusChangedEventHandler.HandleEventAsync(
            new TransactionValidationStatusChangedEvent
            {
    ...
    private async Task<List<Transaction>> GenerateTwoInitializeTransaction()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            var transaction = _osTestHelper.GenerateTransaction(Address.FromPublicKey(newUserKeyPair.PublicKey),
                await _smartContractAddressService.GetAddressByContractNameAsync(
                    await _osTestHelper.GetChainContextAsync(), TokenSmartContractAddressNameProvider.StringName),
    ...
    public async Task CalculateTransactionFee_Success_Test()
    {
        ...
        response.TransactionFeeList[0].ChargingAddress.ShouldBe(transaction.From.ToBase58());
        response.TransactionFeeList[0].Fee.First().Key.ShouldBe("ELF");
        response.TransactionFeeList[0].Fee.First().Value.ShouldBeGreaterThan(10000000L);
