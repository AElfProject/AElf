...
public partial class GenesisContractAuthTest
{
    ...
    public async Task DeploySmartContract_Deterministic_Test()
    {
        ...
        var contractAddress2 = await DeployAsync(SideChainTester, SideParliamentAddress, SideBasicContractZeroAddress, contractDeploymentInput2);
    ...
    [Fact]
    public async Task DeploySmartContract_Deterministic_Test_ContractOperation_Fail()
    {
        var code = ByteString.CopyFrom(Codes.Single(kv => kv.Key.Contains("TestContract.BasicFunction")).Value);
        var mainChainId = Tester.GetChainAsync().Result.Id;
        
        var result = await DeployWithResultAsync(Tester, ParliamentAddress, BasicContractZeroAddress, new ContractDeploymentInput
        {
            Category = KernelConstants.DefaultRunnerCategory, // test the default runner
            Code = code,
            ContractOperation = new ContractOperation()
        ...
        result = await DeployWithResultAsync(Tester, ParliamentAddress, BasicContractZeroAddress, new ContractDeploymentInput
        {
            Category = KernelConstants.DefaultRunnerCategory, // test the default runner
            Code = code,
            ContractOperation = new ContractOperation
    ...
    public async Task DeploySmartContract_Deterministic_Delegate_Test()
    {
        ...
        var result = await DeployWithResultAsync(Tester, ParliamentAddress, BasicContractZeroAddress, contractDeploymentInput);
        result.Error.ShouldContain("Wrong signature.");
        ...
        var output = await tester.CallContractMethodAsync(BasicContractZeroAddress,
            nameof(ACS0Container.ACS0Stub.GetDelegateSignatureAddress), new Empty());
        ...
        await tester.ExecuteContractWithMiningAsync(BasicContractZeroAddress,
            nameof(ACS0Container.ACS0Stub.SetDelegateSignatureAddress), SignerAddress);
        result = await DeployWithResultAsync(Tester, ParliamentAddress, BasicContractZeroAddress, contractDeploymentInput);
        result.Status.ShouldBe(TransactionResultStatus.Mined);
    }
