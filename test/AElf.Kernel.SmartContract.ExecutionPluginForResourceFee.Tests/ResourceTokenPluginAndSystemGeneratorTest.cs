...
namespace AElf.Kernel.SmartContract.ExecutionPluginForResourceFee.Tests
{
    public partial class ExecutionPluginForResourceFeeTest
    {
        ...
        public async Task GenerateTransactions_NoneTransaction_Generate_Test()
        {
            ...
            var systemTransactionGenerator = systemTransactionGenerators.SingleOrDefault(p => p.GetType() == typeof(DonateResourceTransactionGenerator));
            ...
            transactions = await systemTransactionGenerator.GenerateTransactionsAsync(DefaultSender, blockHeader.Height, blockHeader.GetHash());
        ...
        public async Task GenerateTransactions_Success_Test()
        {
            ...
            donateResourceTransaction.MethodName.ShouldBe(nameof(TokenContractContainer.TokenContractStub.DonateResourceToken));
            ...
            var transactionValidations = Application.ServiceProvider.GetRequiredService<IEnumerable<IBlockValidationProvider>>()
                .ToLookup(p => p.GetType()).Select(coll => coll.First()); // One instance per type
            var claimTransactionValidation = transactionValidations.SingleOrDefault(p => p.GetType() == typeof(DonateResourceTokenValidationProvider));
        ...
        public async Task GetPreTransactions_None_PreTransaction_Generate_Test()
        {
            var plugins = Application.ServiceProvider.GetRequiredService<IEnumerable<IPreExecutionPlugin>>()
                .ToLookup(p => p.GetType()).Select(coll => coll.First()); // One instance per type
            var plugin = plugins.SingleOrDefault(p => p.GetType() == typeof(ResourceConsumptionPreExecutionPlugin));
        ...
        {
            var plugins = Application.ServiceProvider.GetRequiredService<IEnumerable<IPostExecutionPlugin>>()
                .ToLookup(p => p.GetType()).Select(coll => coll.First()); // One instance per type
            var plugin = plugins.SingleOrDefault(p => p.GetType() == typeof(ResourceConsumptionPostExecutionPlugin));
            plugin.ShouldNotBeNull();
            var bcs = Application.ServiceProvider.GetRequiredService<IBlockchainService>();
            var chain = await bcs.GetChainAsync();
