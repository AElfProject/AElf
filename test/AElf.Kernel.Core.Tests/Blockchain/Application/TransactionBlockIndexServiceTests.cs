...
namespace AElf.Kernel.Blockchain.Application
{
    public class TransactionBlockIndexServiceTests : AElfKernelWithChainTestBase
    {
        ...
        public async Task UpdateOneBlockIndexWithoutBestChain()
        {
            ...
            var block = _kernelTestHelper.GenerateBlock(previousBlockHeader.Height, HashHelper.ComputeFromString("PreBlockHash"));
            var chain = await _blockchainService.GetChainAsync();
            await AddBlockAsync(chain, block);

            var txId = HashHelper.ComputeFromString("Transaction");
        ...
        {
            var previousBlockHeader = _kernelTestHelper.BestBranchBlockList.Last().Header;
            var block1 = _kernelTestHelper.GenerateBlock(previousBlockHeader.Height, HashHelper.ComputeFromString("PreBlockHash1"));
            var block2 = _kernelTestHelper.GenerateBlock(previousBlockHeader.Height, HashHelper.ComputeFromString("PreBlockHash2"));
