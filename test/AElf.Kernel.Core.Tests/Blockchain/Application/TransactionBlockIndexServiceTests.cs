...
namespace AElf.Kernel.Blockchain.Application
{
    public class TransactionBlockIndexServiceTests : AElfKernelWithChainTestBase
    {
        ...
        public async Task UpdateOneBlockIndexWithoutBestChain()
        {
            ...
            var block = _kernelTestHelper.GenerateBlock(previousBlockHeader.Height, Hash.ComputeFrom("PreBlockHash"));
            var chain = await _blockchainService.GetChainAsync();
            await AddBlockAsync(chain, block);

            var txId = Hash.ComputeFrom("Transaction");
        ...
        {
            var previousBlockHeader = _kernelTestHelper.BestBranchBlockList.Last().Header;
            var block1 = _kernelTestHelper.GenerateBlock(previousBlockHeader.Height, Hash.ComputeFrom("PreBlockHash1"));
            var block2 = _kernelTestHelper.GenerateBlock(previousBlockHeader.Height, Hash.ComputeFrom("PreBlockHash2"));
