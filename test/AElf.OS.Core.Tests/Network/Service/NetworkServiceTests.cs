    ...
    public class NetworkServiceTests : OSCoreNetworkServiceTestBase
    {
        ...
        public async Task GetBlocks_FromNullPeerOrUnfindable_ThrowsException()
        {
            var exceptionNullPeer = await Assert.ThrowsAsync<InvalidOperationException>(async () =>
                    await _networkService.GetBlocksAsync(Hash.ComputeFrom("bHash1"), 1, null));
            
            exceptionNullPeer.Message.ShouldBe("Could not find peer .");

            string peerName = "peer_name";
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(async () =>
                    await _networkService.GetBlocksAsync(Hash.ComputeFrom("bHash1"), 1, peerName));
        ...
        {
            var response = await _networkService.GetBlocksAsync(Hash.ComputeFrom("block_hash"), 1, "failed_peer");
        ...
        {
            var exceptionNullPeer = await Assert.ThrowsAsync<InvalidOperationException>(async () =>
                await _networkService.GetBlockByHashAsync(Hash.ComputeFrom("bHash1"), null));
            
            exceptionNullPeer.Message.ShouldBe("Could not find peer .");
            
            string peerName = "peer_name";
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(async () =>
                await _networkService.GetBlockByHashAsync(Hash.ComputeFrom("bHash1"), peerName));
        ...
        {
            var block = await _networkService.GetBlockByHashAsync(Hash.ComputeFrom("bHash1"), "p1");
        ...
        {
            var blockHeader = _kernelTestHelper.GenerateBlock(10, Hash.ComputeFrom("test")).Header;
            ...
            //broadcast again
            blockHeader = _kernelTestHelper.GenerateBlock(11, Hash.ComputeFrom("new")).Header;
            await _networkService.BroadcastAnnounceAsync(blockHeader);
        }
        ...
        public void BroadcastBlockWithTransactionsAsync_Test()
        {
            ...
            {
                Header = _kernelTestHelper.GenerateBlock(10, Hash.ComputeFrom("test")).Header,
               Transactions =
               {
                   new Transaction(),
                   new Transaction()
