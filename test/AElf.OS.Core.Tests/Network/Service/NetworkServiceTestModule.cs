        ...
        public override void ConfigureServices(ServiceConfigurationContext context)
        {
            ...
            peerPoolMock.Setup(p => p.FindPeerByPublicKey(It.Is<string>(adr => adr == "p1")))
                .Returns<string>(adr =>
                {
                    ...
                    p1.Setup(p => p.GetBlockByHashAsync(It.Is<Hash>(h => h == Hash.ComputeFrom("bHash1"))))
            ...
            peerPoolMock.Setup(p => p.GetPeers(It.IsAny<bool>()))
                .Returns<bool>(includeFailing =>
                {
                    ...
                    p2.Setup(p => p.GetBlocksAsync(It.Is<Hash>(h => h == Hash.ComputeFrom("block")), It.IsAny<int>()))
                        .Returns<Hash, int>((h, cnt) => Task.FromResult(new List<BlockWithTransactions> { blockWithTransactions }));
                    p2.Setup(p => p.GetBlockByHashAsync(It.Is<Hash>(h => h == Hash.ComputeFrom("block"))))
                        .Returns<Hash>(h => Task.FromResult(blockWithTransactions));
                    ...
                    p3.Setup(p => p.GetBlocksAsync(It.Is<Hash>(h => h == Hash.ComputeFrom("blocks")), It.IsAny<int>()))
                        .Returns<Hash, int>((h, cnt) => Task.FromResult(new List<BlockWithTransactions> { blockWithTransactions, blockWithTransactions }));
                    p3.Setup(p => p.GetBlockByHashAsync(It.Is<Hash>(h => h == Hash.ComputeFrom("bHash2"))))
                        .Returns<Hash>(h => Task.FromResult(blockWithTransactions));
                    peers.Add(p3.Object);
