    ...
    public class CrossChainMemoryCacheTest : CrossChainTestBase
    {
        ...
        public void TryAdd_SingleThread_Success()
        {
            ...
            var res = blockInfoCache.TryAdd(new SideChainBlockData
            {
                Height = height,
                ChainId = chainId,
                TransactionStatusMerkleTreeRoot = Hash.ComputeFrom(height.ToString())
            });
            Assert.True(res);
        ...
        public void TryAdd_MultiThreads_WithDifferentData()
        {
            ...
            while (i < 5)
            {
                ...
                var t = Task.Run(() => blockInfoCache.TryAdd(new SideChainBlockData
                {
                    Height = 2 * j + 1,
                    ChainId = chainId,
                    TransactionStatusMerkleTreeRoot = Hash.ComputeFrom((2 * j + 1).ToString())
                }));
                taskList.Add(t);
                i++;
            }
        ...
        public void TryAdd_DataNotContinuous()
        {
            ...
            blockInfoCache.TryAdd(new SideChainBlockData
            {
                Height = 2,
                ChainId = chainId,
                TransactionStatusMerkleTreeRoot = Hash.ComputeFrom("2")
            });
            ...
            blockInfoCache.TryAdd(new SideChainBlockData
            {
                Height = 4,
                ChainId = chainId,
                TransactionStatusMerkleTreeRoot = Hash.ComputeFrom("4")
            });
        ...
        public void TryAdd_MultiThreads_WithSameData()
        {
            ...
            while (i++ < 5)
            {
                var t = Task.Run(() => blockInfoCache.TryAdd(new SideChainBlockData
                {
                    Height = initTarget,
                    ChainId = chainId,
                    TransactionStatusMerkleTreeRoot = Hash.ComputeFrom(initTarget.ToString())
                }));
                taskList.Add(t);
            }
