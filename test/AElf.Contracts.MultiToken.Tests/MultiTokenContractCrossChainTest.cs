...
namespace AElf.Contracts.MultiToken
{
    ...
    public async Task MainChain_RegisterCrossChainTokenContractAddress_Test()
    {
        ...
        // Main chain register
        var result = await RegisterSideChainTokenContractAsync(TokenContractAddress, validateTransaction, merklePath,
    ...
    public async Task MainChain_CrossChainCreateToken_Test()
    {
        ...
        var tokenValidationTransaction = CreateTokenInfoValidationTransaction(createdTokenInfo, SideChainTokenContractStub);
    ...
    public async Task SideChain_CrossChainCreateToken_Test()
    {
        ...
        var executionResult = await SideChainTokenContractStub.CrossChainCreateToken.SendAsync(crossChainCreateTokenInput);
        executionResult.TransactionResult.Status.ShouldBe(TransactionResultStatus.Mined, executionResult.TransactionResult.Error);
    ...
    public async Task SideChain_CrossChainSideChainCreateToken_Test()
    {
        ...
        var executionResult = await SideChain2TokenContractStub.CrossChainCreateToken.SendAsync(crossChainCreateTokenInput);
        executionResult.TransactionResult.Status.ShouldBe(TransactionResultStatus.Mined, executionResult.TransactionResult.Error);
    ...
    public async Task CrossChainCreateToken_WithoutRegister_Test()
    {
        ...
        var executionResult = await SideChainTokenContractStub.CrossChainCreateToken.SendWithExceptionAsync(crossChainCreateTokenInput);
    ...
    public async Task MainChain_CrossChainTransfer_NativeToken_Test()
    {
        ...
        Assert.True(executionResult.TransactionResult.Status == TransactionResultStatus.Mined, executionResult.TransactionResult.Error);
    ...
    public async Task MainChain_CrossChainTransfer_Test()
    {
        ...
        var tokenValidationTransaction = TokenContractStub.ValidateTokenInfoExists.GetTransaction(new ValidateTokenInfoExistsInput
        {
    ...
    public async Task MainChain_CrossChainTransfer_IncorrectChainId_Test()
    {
        ...
        var executionResult = await TokenContractStub.CrossChainTransfer.SendWithExceptionAsync(new CrossChainTransferInput
        {
            Symbol = NativeToken,
            ToChainId = sideChainId,
            Amount = 1000,
    ...
    public async Task SideChain_CrossChainReceived_NativeToken_Test()
    {
        ...
        //Main chain cross transfer to side chain
        var crossChainTransferTransaction = TokenContractStub.CrossChainTransfer.GetTransaction(new CrossChainTransferInput
        {
            Symbol = NativeToken,
            ToChainId = sideChainId,
            Amount = 1000,
        ...
        var height = blockExecutedSet.Height > _parentChainHeightOfCreation ? blockExecutedSet.Height : _parentChainHeightOfCreation;
    ...
    public async Task SideChain_CrossChainReceived_Twice_Test()
    {
        ...
        var executionResult = await SideChainTokenContractStub.CrossChainReceiveToken.SendWithExceptionAsync(crossChainReceiveTokenInput);
        Assert.True(executionResult.TransactionResult.Status == TransactionResultStatus.Failed);
    ...
    public async Task SideChain_CrossChainReceived_WrongReceiver_Test()
    {
        ...
        var tokenContractImplStub = SideChainTestKit.GetTester<TokenContractImplContainer.TokenContractImplStub>(SideTokenContractAddress,
            SampleAccount.Accounts[2].KeyPair);
        var executionResult = await tokenContractImplStub.CrossChainReceiveToken.SendWithExceptionAsync(crossChainReceiveTokenInput);
        Assert.True(executionResult.TransactionResult.Status == TransactionResultStatus.Failed);
    ...
    private Transaction CreateTransactionForTokenCreation(TokenContractImplContainer.TokenContractImplStub tokenContractImplStub,
        Address issuer, string symbol)
    {
    ...
    private async Task RegisterSideChainContractAddressOnSideChainAsync(int sideChainId)
    {
        ...
        var result = await RegisterSideChainTokenContractOnSieChain2Async(Side2TokenContractAddress, validateTransaction1, merklePath,
            boundParentChainHeightAndMerklePath, sideChainId);
        result.Status.ShouldBe(TransactionResultStatus.Mined, result.Error);
    }
        ...
        long height)
    {
        ...
        var proposalId = await CreateProposalAsync(SideChainParliamentContractStub, nameof(TokenContractImplContainer.TokenContractImplStub.RegisterCrossChainTokenContractAddress),
        ...
        return await SideChainTestKit.ExecuteTransactionWithMiningAsync(SideChainParliamentContractStub.Release.GetTransaction(proposalId));;
    }
    
    private async Task<TransactionResult> RegisterSideChainTokenContractAsync(Address tokenContract, IMessage transaction,
        MerklePath merklePath,
        CrossChainMerkleProofContext boundParentChainHeightAndMerklePath, int sideChainId)
    {
        ...
        var proposalId = await CreateProposalAsync(ParliamentContractStub,nameof(TokenContractImplContainer.TokenContractImplStub.RegisterCrossChainTokenContractAddress),
    ...
    private async Task<TransactionResult> RegisterSideChainTokenContractOnSieChain2Async(Address tokenContract, IMessage transaction,
        MerklePath merklePath,
        CrossChainMerkleProofContext boundParentChainHeightAndMerklePath, int sideChainId)
    {
        var registerCrossChainTokenContractAddressInput = new RegisterCrossChainTokenContractAddressInput
        ...
        registerCrossChainTokenContractAddressInput.MerklePath.MerklePathNodes.AddRange(
            boundParentChainHeightAndMerklePath.MerklePathFromParentChain.MerklePathNodes);
        var proposalId = await CreateProposalAsync(SideChain2ParliamentContractStub,nameof(TokenContractImplContainer.TokenContractImplStub.RegisterCrossChainTokenContractAddress),
            registerCrossChainTokenContractAddressInput.ToByteString(), tokenContract);
        await ApproveWithMinersAsync(proposalId);
    ...
    private async Task IndexMainChainTransactionAsync(long height, Hash txRoot, Hash blockRoot)
    {
        ...
        await BootMinerChangeRoundAsync(SideChainAEDPoSContractStub, false);
        var proposingResult = await SideChainCrossChainContractStub.ProposeCrossChainIndexing.SendAsync(crossChainBlockData);
    ...
    private async Task IndexMainChainTransactionOnSideChain2Async(long height, Hash txRoot, Hash blockRoot)
    {
        ...
        var proposingResult = await SideChain2CrossChainContractStub.ProposeCrossChainIndexing.SendAsync(crossChainBlockData);
        var proposalId = ProposalCreated.Parser
            .ParseFrom(proposingResult.TransactionResult.Logs.First(l => l.Name.Contains(nameof(ProposalCreated)))
                .NonIndexed)
            .ProposalId;
    ...
    private async Task<Hash> IndexMainChainBlockAsync(Transaction transaction,long height,long sideHeight,List<Hash> indexedSideChainBlockRoots)
    {
        var fakeHash1 = HashHelper.ComputeFrom("fake1");
        var fakeHash2 = HashHelper.ComputeFrom("fake2");
        ...
        await BootMinerChangeRoundAsync(SideChainAEDPoSContractStub, false);
        var proposingExecutionResult = await SideChainCrossChainContractStub.ProposeCrossChainIndexing.SendAsync(crossChainBlockData);
        var proposalId = ProposalCreated.Parser
