...
namespace AElf.Contracts.Parliament
{
    public class ParliamentContractTest : ParliamentContractTestBase
    {
        ...
        {
            var proposalOutput = await ParliamentContractStub.GetProposal.CallAsync(Hash.ComputeFrom("Test"));
        ...
        {
            var transactionResult =
                await ParliamentContractStub.Approve.SendWithExceptionAsync(Hash.ComputeFrom("Test"));
            transactionResult.TransactionResult.Status.ShouldBe(TransactionResultStatus.Failed);
        }
