...
public partial class NFTMarketContract
{
    ...
    {
        return State.WhiteListAddressPriceListMap[input.Symbol][input.TokenId][input.Owner];
    }
    ...
    {
        if (input.Address != null) return State.OfferListMap[input.Symbol][input.TokenId][input.Address];
        ...
        foreach (var address in addressList.Value)
        {
            ...
            if (offerList != null) allOfferList.Value.Add(offerList.Value);
        }
    ...
    public override BidList GetBidList(GetBidListInput input)
    {
        ...
        foreach (var address in addressList.Value)
        {
            ...
            if (bid != null) allBidList.Value.Add(bid);
        }
    ...
    public override Price GetStakingTokens(StringValue input)
    {
        ...
        if (customizeInfo == null) return new Price();
    ...
    public override RoyaltyInfo GetRoyalty(GetRoyaltyInput input)
    {
        ...
        if (input.TokenId != 0)
        {
            ...
            if (certainNftRoyalty.IsManuallySet) royaltyInfo.Royalty = certainNftRoyalty.Royalty;
        }
