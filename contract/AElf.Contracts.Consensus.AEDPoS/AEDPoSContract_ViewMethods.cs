    ...
    public partial class AEDPoSContract
    {
        ...
        public override MinerList GetCurrentMinerList(Empty input) =>
            TryToGetCurrentRoundInformation(out var round)
                ? new MinerList
                {
                    Pubkeys =
                    {
                        round.RealTimeMinersInformation.Keys.Select(k => k.FromHexString())
                    }
                }
                : new MinerList();
        ...
        private Round GenerateFirstRoundOfNextTerm(string senderPubkey, int miningInterval)
        {
            ...
            else
            {
                ...
                miners.Pubkeys.AddRange(currentRound.RealTimeMinersInformation.Keys.Select(k => k.FromHexString()));
