...
namespace AElf.RuntimeSetup
{
    ...
    public class RuntimeSetupAElfModule : AElfModule
    {
        public override void ConfigureServices(ServiceConfigurationContext context)
        {
            ...
            var keys = variables.Keys.Cast<string>().Select(p => new {Ori = p, Uni = p.ToUpper()}).GroupBy(p => p.Uni)
                .ToList();
            foreach (var key in keys)
            {
                if (key.Count() <= 1) continue;
                var list = key.ToList();
                for (var i = 1; i < list.Count; i++) Environment.SetEnvironmentVariable(list[i].Ori, null);
            }

            var log4NetConfigFile =
                Path.Combine(context.Services.GetHostingEnvironment().ContentRootPath, "log4net.config");
