...
namespace AElf.Kernel.SmartContractExecution.Scheduling
{
    public class GrouperTests : SmartContractExecutionGrouperTestBase
    {
        public GrouperTests()
        {
            ...
            for (var i = 0; i < 30; i++) _accountList.Add(Address.Generate());
        }

        private readonly Grouper _grouper;

        private readonly int _chainId = 1;
        private readonly List<Address> _accountList = new List<Address>();
        ...
        private List<Transaction> GetFullTxList()
        {
            ...
            for (int i = 0, x = 1; i < txList2.Count; i++, x += 3) txList1.Insert(i + x, txList2[i]);
