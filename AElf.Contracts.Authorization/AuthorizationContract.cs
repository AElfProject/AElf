...
namespace AElf.Contracts.Authorization
{
    public partial class AuthorizationContract : AuthorizationContractContainer.AuthorizationContractBase
    {
        public override BoolValue IsMultiSigAccount(Address input)
        {
            ...
            if (address.Equals(Context.Genesis) || authorization != null) return new BoolValue {Value = true};
        ...
        private Transaction CheckAndFillTxnData(Proposal proposal, Approved approved)
        {
            ...
            foreach (var approval in approved.Approvals) proposedTxn.Sigs.Add(approval.Signature);
        ...
        private void CheckAuthority(Proposal proposal, Kernel.Authorization authorization)
        {
            if (authorization.ProposerThreshold > 0)
            {
                ...
                var reviewer = authorization.Reviewers.FirstOrDefault(r =>
        ...
        {
            var toSig = proposal.TxnData.ToByteArray().CalculateHash();
        ...
        public override Proposal GetProposal(Hash input)
        {
            ...
            if (proposal.Status == ProposalStatus.Released) return proposal;
        ...
        public override Address CreateMultiSigAccount(Kernel.Authorization input)
        {
            ...
            var multiSigAccount = authorization.MultiSigAccount ??
            ...
            var canBeProposed = authorization.Reviewers.Any(r => r.Weight >= authorization.ProposerThreshold);
        ...
        public override Hash Propose(Proposal input)
        {
            ...
            var timestamp = proposal.ExpiredTime.ToDateTime();
        ...
        public override BoolValue SayYes(Approval input)
        {
            var approval = input;
            // check validity of proposal 
            var hash = approval.ProposalHash;
            ...
            var toSig = proposal.TxnData.ToByteArray().CalculateHash();
            var pubKey = Context.RecoverPublicKey(approval.Signature.ToByteArray(), toSig);
            ...
            if (CheckApproval(approved, authorization, proposal)) State.Proposals[hash] = proposal;
