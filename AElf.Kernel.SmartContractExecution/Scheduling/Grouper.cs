...
namespace AElf.Kernel.SmartContractExecution.Scheduling
{
    ...
    {
        private readonly IResourceUsageDetectionService _resourceUsageDetectionService;
        ...
        public async Task<Tuple<List<List<Transaction>>, Dictionary<Transaction, Exception>>> ProcessNaive(int chainId,
            List<Transaction> transactions)
        {
            var txResourceHandle = new Dictionary<Transaction, string>();
            var failedTxs = new Dictionary<Transaction, Exception>();
            if (transactions.Count == 0)
                return new Tuple<List<List<Transaction>>, Dictionary<Transaction, Exception>>(
                    new List<List<Transaction>>(), failedTxs);

            var resourceUnionSet = new Dictionary<string, UnionFindNode>();
            ...
            foreach (var tx in transactions)
                ...
                {
                    var nodeId = resourceUnionSet[firstResource].Find().NodeId;
                ...
                {
                    if (!failedTxs.ContainsKey(tx)) result.Add(new List<Transaction> {tx});
                }

            result.AddRange(grouped.Values);

            Logger.LogInformation(string.Format(
                "Grouper on chainId \"{0}\" group [{1}] transactions into [{2}] groups with sizes [{3}], There are also {4} transactions failed retriving resource",
        ...
        public async Task<Tuple<List<List<Transaction>>, Dictionary<Transaction, Exception>>> ProcessWithCoreCount(
            GroupStrategy strategy, int totalCores, int chainId, List<Transaction> transactions)
        {
            if (strategy == GroupStrategy.NaiveGroup) return await ProcessNaive(chainId, transactions);

            if (transactions.Count == 0)
                return new Tuple<List<List<Transaction>>, Dictionary<Transaction, Exception>>(
                    new List<List<Transaction>>(), new Dictionary<Transaction, Exception>());

            if (totalCores <= 0) throw new InvalidParameterException("Total core count " + totalCores + " is invalid");
            ...
            return new Tuple<List<List<Transaction>>, Dictionary<Transaction, Exception>>(mergedGroups,
                groupResults.Item2);
        }
        ...
        /// <returns></returns>
        public List<List<Transaction>> ProcessWithCoreCount_MinsAddUp(int totalCores,
            List<List<Transaction>> unmergedGroups)
        {
            ...
            while (sortedList.Count > totalCores)
            {
                ...
                while (index >= 0 && sortedList[index].Count < subMin.Count) index--;
        ...
        ///     then fetch next biggest group in group result, merge smallest groups untill the transactions count in group reach
        ///     threshold
        ...
        ///     This func Will produce group in first step: [499, 499, 498, 498, 6], next step will be [1002, 499, 499] which is
        ///     unbalanced
        /// </summary>
        /// <param name="totalCores"></param>
        /// <param name="unmergedGroups"></param>
        ...
        public List<List<Transaction>> ProcessWithCoreCount_MaxAddMins(int totalCores,
            List<List<Transaction>> unmergedGroups)
        {
            var sortedUnmergedGroups = unmergedGroups.OrderByDescending(a => a.Count).ToList();

            if (sortedUnmergedGroups.Count == 0) return new List<List<Transaction>>();

            var transactionCount = sortedUnmergedGroups.SelectMany(a => a).Count();
            ...
            var resGroupCount = totalCores + 0;
            var mergeThreshold = transactionCount / resGroupCount;
            ...
            while (startIndex <= endIndex)
            {
                ...
                while (tempList.Count + sortedUnmergedGroups.ElementAt(endIndex).Count <= mergeThreshold &&
                       startIndex < endIndex)
                {
            ...
            if (res.Count > resGroupCount)
            {
                ...
                for (index = 0; index <= temp.Count - resGroupCount; index++) merge.AddRange(temp[index]);
                res.Add(merge);
                for (; index < temp.Count; index++) res.Add(temp[index]);
            }
