...
namespace AElf.OS.Network.Grpc.Connection
{
    public class ConnectionService : IConnectionService
    {
        ...
        private ConnectError ValidateConnectionInfo(IPEndPoint endpoint, ConnectionInfo connectionInfo)
        {
            ...
            {
                int peerFromIp = _peerPool.GetPeersByIpAddress(endpoint.Address).Count;
                if (peerFromIp >= NetworkOptions.MaxPeersPerIpAddress)
                {
                    Logger.LogWarning($"Max peers from {endpoint.Address} exceeded, current count {peerFromIp} (max. per ip {NetworkOptions.MaxPeersPerIpAddress}).");
                    return ConnectError.ConnectionRefused;
                }
            }
