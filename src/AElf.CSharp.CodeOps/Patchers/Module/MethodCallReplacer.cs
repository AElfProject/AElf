...
public class MethodCallReplacer : IPatcher<ModuleDefinition>
{
    ...
    private static readonly Dictionary<string, string> MethodCallsToReplace = new()
    {
    ...
    private static readonly Dictionary<OpCode, OpCode> OpCodesToReplace = new()
    {
    ...
    public void Patch(ModuleDefinition module)
    {
        ...
        // Patch the types
        foreach (var typ in module.Types) PatchType(typ, sdkTypeDefs);
    }

    private void PatchType(TypeDefinition typ, Dictionary<string, TypeDefinition> sdkTypeDefs)
    {
        // Patch the methods in the type
        foreach (var method in typ.Methods) PatchMethod(method, sdkTypeDefs);

        // Patch if there is any nested type within the type
        foreach (var nestedType in typ.NestedTypes) PatchType(nestedType, sdkTypeDefs);
    }
    ...
    private void PatchMethod(MethodDefinition method, Dictionary<string, TypeDefinition> sdkTypeDefs)
    {
        ...
        var methodCallsToReplace = method.Body.Instructions.Where(i =>
                ...
                MethodCallsToReplace.Any(m => ((MethodReference)i.Operand).FullName.Contains(m.Key)))
            .ToList();
    ...
    private MethodReference GetSdkMethodReference(Dictionary<string, TypeDefinition> sdkTypeDefs,
