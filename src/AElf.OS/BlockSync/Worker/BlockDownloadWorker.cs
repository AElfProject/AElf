...
public partial class BlockDownloadWorker : AsyncPeriodicBackgroundWorkerBase
{
    ...
    internal async Task ProcessDownloadJobAsync()
    {
        while (true)
        {
            ...
            if (!await PerformDownloading(jobInfo, chain))
            {
                return;
            }
        }
    ...
    [ExceptionHandler(typeof(Exception), TargetType = typeof(BlockDownloadWorker),
        MethodName = nameof(HandleExceptionWhilePerformingDownloading))]
    private async Task<bool> PerformDownloading(BlockDownloadJobInfo jobInfo, Chain chain)
    {
        if (!ValidateBeforeDownload(jobInfo)) return false;
